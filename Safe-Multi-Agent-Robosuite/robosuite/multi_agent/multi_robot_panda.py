...
class MujocoMulti(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        if self.env_version == 2:
            ...
            elif self.scenario in ["Door", "Lift", "NutAssembly", "NutAssemblyRound", "NutAssemblySingle",
                   "NutAssemblySquare", "PickPlace", "PickPlaceBread", "PickPlaceCan", "PickPlaceCereal",
                   "PickPlaceMilk", "PickPlaceSingle", "Stack"]:
                this_env = MujocoEnv_()
            elif self.scenario in ["TwoArmHandover", "TwoArmLift", "TwoArmPegInHole"]:
                this_env = MujocoEnv_()
...
#
# class MujocoEnv(MultiAgentEnv):
#     def __init__(self, batch_size=None, **kwargs):
#         super().__init__(batch_size, **kwargs)
#
#         # env_args = {"scenario": "Panda-v2",
#         #             "agent_conf": "7x1",
#         #             "agent_obsk": 1,
#         #             "episode_limit": 1000}
#         # kwargs = env_args
...
#         self.agent_obsk = kwargs["env_args"].get("agent_obsk",
#                                                  None)  # if None, fully observable else k>=0 implies observe nearest k agents or joints
#         # self.agent_obsk = kwargs.get("agent_obsk", None)
#
#         self.agent_obsk_agents = kwargs["env_args"].get("agent_obsk_agents",
#                                                         False)  # observe full k nearest agents (True) or just single joints (False)
#         self.agent_obsk_agents = kwargs.get("agent_obsk_agents", False)
#
#         if self.agent_obsk is not None:
#             # self.k_categories_label = kwargs["env_args"].get("k_categories")
...
#         this_env = MujocoEnv_
#         self.wrapped_env = NormalizedActions(
#             TimeLimit(this_env(kwargs), max_episode_steps=self.episode_limit))
#
#         if self.env_version == 2:
#             try:
#                 self.wrapped_env = NormalizedActions(gym.make(self.scenario))
#                 print("++++++++++++ManyAgentAntEnv____type+++++++++++++", type(gym.make(self.scenario)))
#             except gym.error.Error:  # env not in gym
#                 if self.scenario in ["manyagent_ant"]:
#                     print("++++++++++++ManyAgentAntEnv____type+++++++++++++", type(this_env))
#                 elif self.scenario in ["Panda-Door-v2"]:
#                     this_env = MujocoEnv_
#
#                     print("=============env.action_spec=========", this_env.action_spec)
#
#                     # from envs.robosuite.robosuite.environments.base import MujocoEnv as this_env
#
#                     print("++++++++++++panda____type+++++++++++++", type(this_env))
#                 else:
#                     raise NotImplementedError('Custom env not implemented!')
#
#                 # if self.scenario in ["Panda-Door-v2"]:
#                 #     self.wrapped_env = NormalizedActions_multi_robot(
#                 #         TimeLimit(this_env(**kwargs["env_args"]), max_episode_steps=self.episode_limit))
#                 # else:
#                 #     self.wrapped_env = NormalizedActions(
#                 #         TimeLimit(this_env(**kwargs["env_args"]), max_episode_steps=self.episode_limit))
...
#                     TimeLimit(this_env(kwargs), max_episode_steps=self.episode_limit))
#
#                 # self.wrapped_env = NormalizedActions(this_env)
#         else:
#             assert False, "not implemented!"
#
#         self.timelimit_env = self.wrapped_env.env
